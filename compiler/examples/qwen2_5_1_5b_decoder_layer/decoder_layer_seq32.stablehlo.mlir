module {
  func.func @qwen2_5_1_5b_decoder_layer_seq32(
      ...
      %down_weight: tensor<8960x1536xi8>) -> tensor<32x1536xbf16> {
    ...
    %attention_query_2d = stablehlo.dot_general %rms1_result, %attention_query_weight_bf16,
        ...
        (tensor<32x1536xbf16>, tensor<1536x1536xbf16>) -> tensor<32x1536xbf16>
    ...
    %attention_cos_q = stablehlo.broadcast_in_dim %attention_cos_base, dims = [0, 1, 2] :
        (tensor<32x1x64xf32>) -> tensor<32x12x64xf32>
    ...
    %attention_cos_k = stablehlo.broadcast_in_dim %attention_cos_base, dims = [0, 1, 2] :
        (tensor<32x1x64xf32>) -> tensor<32x2x64xf32>
    ...
    %attention_query_f32 = stablehlo.convert %attention_query_heads :
        (tensor<32x12x128xbf16>) -> tensor<32x12x128xf32>
    %attention_key_f32 = stablehlo.convert %attention_key_heads :
        (tensor<32x2x128xbf16>) -> tensor<32x2x128xf32>
    %attention_query_pairs = stablehlo.reshape %attention_query_f32 :
        (tensor<32x12x128xf32>) -> tensor<32x12x64x2xf32>
    %attention_key_pairs = stablehlo.reshape %attention_key_f32 :
        (tensor<32x2x128xf32>) -> tensor<32x2x64x2xf32>
    %attention_query_even_4d = "stablehlo.slice"(%attention_query_pairs) {
      start_indices = array<i64: 0, 0, 0, 0>,
      limit_indices = array<i64: 32, 12, 64, 1>,
      strides = array<i64: 1, 1, 1, 1>
    } : (tensor<32x12x64x2xf32>) -> tensor<32x12x64x1xf32>
    %attention_query_odd_4d = "stablehlo.slice"(%attention_query_pairs) {
      start_indices = array<i64: 0, 0, 0, 1>,
      limit_indices = array<i64: 32, 12, 64, 2>,
      strides = array<i64: 1, 1, 1, 1>
    } : (tensor<32x12x64x2xf32>) -> tensor<32x12x64x1xf32>
    %attention_key_even_4d = "stablehlo.slice"(%attention_key_pairs) {
      start_indices = array<i64: 0, 0, 0, 0>,
      limit_indices = array<i64: 32, 2, 64, 1>,
      strides = array<i64: 1, 1, 1, 1>
    } : (tensor<32x2x64x2xf32>) -> tensor<32x2x64x1xf32>
    %attention_key_odd_4d = "stablehlo.slice"(%attention_key_pairs) {
      start_indices = array<i64: 0, 0, 0, 1>,
      limit_indices = array<i64: 32, 2, 64, 2>,
      strides = array<i64: 1, 1, 1, 1>
    } : (tensor<32x2x64x2xf32>) -> tensor<32x2x64x1xf32>
    %attention_query_even = stablehlo.reshape %attention_query_even_4d :
        (tensor<32x12x64x1xf32>) -> tensor<32x12x64xf32>
    %attention_query_odd = stablehlo.reshape %attention_query_odd_4d :
        (tensor<32x12x64x1xf32>) -> tensor<32x12x64xf32>
    %attention_key_even = stablehlo.reshape %attention_key_even_4d :
        (tensor<32x2x64x1xf32>) -> tensor<32x2x64xf32>
    %attention_key_odd = stablehlo.reshape %attention_key_odd_4d :
        (tensor<32x2x64x1xf32>) -> tensor<32x2x64xf32>

    %attention_query_even_cos = stablehlo.multiply %attention_query_even, %attention_cos_q : tensor<32x12x64xf32>
    %attention_query_odd_sin = stablehlo.multiply %attention_query_odd, %attention_sin_q : tensor<32x12x64xf32>
    %attention_query_odd_cos = stablehlo.multiply %attention_query_odd, %attention_cos_q : tensor<32x12x64xf32>
    %attention_query_even_sin = stablehlo.multiply %attention_query_even, %attention_sin_q : tensor<32x12x64xf32>
    ...
    %attention_query_rotated_even_4d = stablehlo.reshape %attention_query_rotated_even :
        (tensor<32x12x64xf32>) -> tensor<32x12x64x1xf32>
    ...
    %attention_query_rope_pairs = "stablehlo.concatenate"(
        ...
        (tensor<32x12x64x1xf32>, tensor<32x12x64x1xf32>) -> tensor<32x12x64x2xf32>
    %attention_query_rope_f32 = stablehlo.reshape %attention_query_rope_pairs :
        (tensor<32x12x64x2xf32>) -> tensor<32x12x128xf32>
    %attention_query_rope = stablehlo.convert %attention_query_rope_f32 :
        (tensor<32x12x128xf32>) -> tensor<32x12x128xbf16>

    %attention_key_even_cos = stablehlo.multiply %attention_key_even, %attention_cos_k : tensor<32x2x64xf32>
    %attention_key_odd_sin = stablehlo.multiply %attention_key_odd, %attention_sin_k : tensor<32x2x64xf32>
    %attention_key_odd_cos = stablehlo.multiply %attention_key_odd, %attention_cos_k : tensor<32x2x64xf32>
    %attention_key_even_sin = stablehlo.multiply %attention_key_even, %attention_sin_k : tensor<32x2x64xf32>
    ...
    %attention_key_rotated_even_4d = stablehlo.reshape %attention_key_rotated_even :
        (tensor<32x2x64xf32>) -> tensor<32x2x64x1xf32>
    ...
    %attention_key_rope = stablehlo.reshape %attention_key_grouped :
        (tensor<32x2x6x128xbf16>) -> tensor<32x12x128xbf16>
    ...
    %attention_query_bhsd = stablehlo.transpose %attention_query_rope, dims = [1, 0, 2] :
        (tensor<32x12x128xbf16>) -> tensor<12x32x128xbf16>
    ...
    %attention_scores = stablehlo.dot_general %attention_query_bhsd, %attention_key_bhsd,
        ...
        precision = [] :
        (tensor<12x32x128xbf16>, tensor<12x32x128xbf16>) -> tensor<12x32x32xbf16>
    ...
    %attention_scale_broadcast = stablehlo.broadcast_in_dim %attention_scale, dims = [] :
        (tensor<bf16>) -> tensor<12x32x32xbf16>
    %attention_scaled_scores = stablehlo.multiply %attention_scores, %attention_scale_broadcast :
        tensor<12x32x32xbf16>

    %attention_query_index = stablehlo.iota dim = 1 : tensor<12x32x32xi32>
    %attention_key_index = stablehlo.iota dim = 2 : tensor<12x32x32xi32>
    %attention_causal = stablehlo.compare GE, %attention_query_index, %attention_key_index, SIGNED :
        (tensor<12x32x32xi32>, tensor<12x32x32xi32>) -> tensor<12x32x32xi1>
    ...
    %attention_masked_scores = stablehlo.select %attention_causal, %attention_scaled_scores, %attention_negative_broadcast :
        tensor<12x32x32xi1>, tensor<12x32x32xbf16>
    ...
    %attention_context_bhsd = stablehlo.dot_general %attention_probability, %attention_value_bhsd,
        ...
        precision = [] :
        (tensor<12x32x32xbf16>, tensor<12x32x128xbf16>) -> tensor<12x32x128xbf16>
    %attention_context_shd = stablehlo.transpose %attention_context_bhsd, dims = [1, 0, 2] :
        (tensor<12x32x128xbf16>) -> tensor<32x12x128xbf16>
    %attention_context = stablehlo.reshape %attention_context_shd :
        (tensor<32x12x128xbf16>) -> tensor<32x1536xbf16>
    ...
    %ffn_x_f = stablehlo.convert %rms2_result : (tensor<32x1536xbf16>) -> tensor<32x1536xf32>
    ...
    %ffn_up = stablehlo.dot_general %ffn_x_f, %ffn_up_w_f,
      contracting_dims = [1] x [0], precision = [DEFAULT, DEFAULT]
      : (tensor<32x1536xf32>, tensor<1536x8960xf32>) -> tensor<32x8960xf32>
    %ffn_sigmoid = stablehlo.logistic %ffn_gate : tensor<32x8960xf32>
    %ffn_silu = stablehlo.multiply %ffn_gate, %ffn_sigmoid : tensor<32x8960xf32>
    %ffn_hidden = stablehlo.multiply %ffn_silu, %ffn_up : tensor<32x8960xf32>
    %ffn_down = stablehlo.dot_general %ffn_hidden, %ffn_down_w_f,
      contracting_dims = [1] x [0], precision = [DEFAULT, DEFAULT]
      : (tensor<32x8960xf32>, tensor<8960x1536xf32>) -> tensor<32x1536xf32>
    %ffn_result = stablehlo.convert %ffn_down : (tensor<32x1536xf32>) -> tensor<32x1536xbf16>
