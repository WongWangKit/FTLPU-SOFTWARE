module {
  func.func @smollm2_attention(
      %input: tensor<128x576xbf16>,
      %query_weight: tensor<576x576xi8>,
      %key_weight: tensor<576x192xi8>,
      %value_weight: tensor<576x192xi8>,
      %output_weight: tensor<576x576xi8>) -> tensor<128x576xbf16> {
    %query_weight_bf16 = stablehlo.convert %query_weight :
        (tensor<576x576xi8>) -> tensor<576x576xbf16>
    %key_weight_bf16 = stablehlo.convert %key_weight :
        (tensor<576x192xi8>) -> tensor<576x192xbf16>
    %value_weight_bf16 = stablehlo.convert %value_weight :
        (tensor<576x192xi8>) -> tensor<576x192xbf16>
    %output_weight_bf16 = stablehlo.convert %output_weight :
        (tensor<576x576xi8>) -> tensor<576x576xbf16>

    %query_2d = stablehlo.dot_general %input, %query_weight_bf16,
        contracting_dims = [1] x [0], precision = [] :
        (tensor<128x576xbf16>, tensor<576x576xbf16>) -> tensor<128x576xbf16>
    %key_2d = stablehlo.dot_general %input, %key_weight_bf16,
        contracting_dims = [1] x [0], precision = [] :
        (tensor<128x576xbf16>, tensor<576x192xbf16>) -> tensor<128x192xbf16>
    %value_2d = stablehlo.dot_general %input, %value_weight_bf16,
        contracting_dims = [1] x [0], precision = [] :
        (tensor<128x576xbf16>, tensor<576x192xbf16>) -> tensor<128x192xbf16>
    ...
    %scale = stablehlo.constant dense<1.250000e-01> : tensor<bf16>
    ...
    %negative = stablehlo.constant dense<0xFF80> : tensor<bf16>
    ...
    %negative_init = stablehlo.constant dense<0xFF80> : tensor<bf16>
    %row_max = "stablehlo.reduce"(%masked_scores, %negative_init) ({
      ^bb0(%lhs: tensor<bf16>, %rhs: tensor<bf16>):
        %max = stablehlo.maximum %lhs, %rhs : tensor<bf16>
    ...
    %centered = stablehlo.subtract %masked_scores, %row_max_broadcast :
        tensor<9x128x128xbf16>
    %exp = stablehlo.exponential %centered : tensor<9x128x128xbf16>
    %zero = stablehlo.constant dense<0.000000e+00> : tensor<bf16>
    %row_sum = "stablehlo.reduce"(%exp, %zero) ({
      ^bb0(%lhs: tensor<bf16>, %rhs: tensor<bf16>):
        %sum = stablehlo.add %lhs, %rhs : tensor<bf16>
    ...
    %result = stablehlo.dot_general %context, %output_weight_bf16,
        contracting_dims = [1] x [0], precision = [] :
        (tensor<128x576xbf16>, tensor<576x576xbf16>) -> tensor<128x576xbf16>
