module {
  func.func @smollm2_135m_decoder_layer_seq128(
    ...
    %rms1_hidden = stablehlo.constant dense<5.760000e+02> : tensor<f32>
    ...
    %rms1_epsilon = stablehlo.constant dense<1.000000e-05> : tensor<f32>
    ...
    %attention_frequency_scale = stablehlo.constant dense<-3.125000e-02> : tensor<f32>
    ...
    %attention_angle = stablehlo.multiply %attention_position_matrix, %attention_frequency_matrix :
        tensor<128x32xf32>
    %attention_cos = stablehlo.cosine %attention_angle : tensor<128x32xf32>
    %attention_sin = stablehlo.sine %attention_angle : tensor<128x32xf32>
    ...
    %attention_query_even_cos = stablehlo.multiply %attention_query_even, %attention_cos_q : tensor<128x9x32xf32>
    %attention_query_odd_sin = stablehlo.multiply %attention_query_odd, %attention_sin_q : tensor<128x9x32xf32>
    %attention_query_odd_cos = stablehlo.multiply %attention_query_odd, %attention_cos_q : tensor<128x9x32xf32>
    %attention_query_even_sin = stablehlo.multiply %attention_query_even, %attention_sin_q : tensor<128x9x32xf32>
    ...
    %attention_key_odd_cos = stablehlo.multiply %attention_key_odd, %attention_cos_k : tensor<128x3x32xf32>
    %attention_key_even_sin = stablehlo.multiply %attention_key_even, %attention_sin_k : tensor<128x3x32xf32>
    ...
    %rms2_sum = "stablehlo.reduce"(%rms2_square, %rms2_zero) ({
      ^bb0(%rms2_lhs: tensor<f32>, %rms2_rhs: tensor<f32>):
        ...
        (tensor<128x576xf32>, tensor<f32>) -> tensor<128xf32>
    %rms2_hidden = stablehlo.constant dense<5.760000e+02> : tensor<f32>
    ...
    %rms2_epsilon = stablehlo.constant dense<1.000000e-05> : tensor<f32>
    ...
    %ffn_up = stablehlo.dot_general %ffn_x_f, %ffn_up_w_f,
      contracting_dims = [1] x [0], precision = [DEFAULT, DEFAULT]
      : (tensor<128x576xf32>, tensor<576x1536xf32>) -> tensor<128x1536xf32>
    %ffn_sigmoid = stablehlo.logistic %ffn_gate : tensor<128x1536xf32>
    %ffn_silu = stablehlo.multiply %ffn_gate, %ffn_sigmoid : tensor<128x1536xf32>
    %ffn_hidden = stablehlo.multiply %ffn_silu, %ffn_up : tensor<128x1536xf32>
