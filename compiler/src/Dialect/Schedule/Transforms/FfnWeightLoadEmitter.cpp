...
MxmLoadOp emitFfnWeightTile(
  ...
  if (localDequant) {
    ...
    for (int64_t stream = 0; stream < rawRoute.getStreamCount(); ++stream) {
      ...
      auto read = rewriter.create<MemReadOp>(
          location, rawRoute.getInput(), startCycle - latency, duration,
          streamBase + stream, 1,
    ...
    auto load = rewriter.create<MxmLoadOp>(
        location, readValue, startCycle, duration, streamBase,
        throughput.mxm_int8_load_streams_per_cycle, unit, weightBuffer);
  ...
  for (int64_t stream = 0; stream < rawRoute.getStreamCount(); ++stream) {
    ...
    auto read = rewriter.create<MemReadOp>(
        location, rawRoute.getInput(), startCycle - latency, duration, stream,
  ...
  for (int64_t stream = 0; stream < rawRoute.getStreamCount(); ++stream) {
    value = create_vxm(rewriter, location, readValue, readValue,
                       ...
                       "immediate", 0, scale, "fp32", -1, duration, 1, hemi,
                       hemi, functionArgumentIndex(rawRoute.getInput()))
                .getResult();
    value = create_vxm(rewriter, location, value, readValue, dequantizedType,
                       startCycle + 1, 8 + stream, "cast", "alu", stream, 0.0f,
                       "immediate", 0, 0.0f, dataFormat,
                       localMxm * throughput.mxm_load_streams_per_cycle +
                           stream * 2,
                       duration, 1, hemi, hemi)
                .getResult();
  }

  return rewriter.create<MxmLoadOp>(
      location, value, startCycle + throughput.vxm_weight_to_iw_latency,
      duration, 0, throughput.mxm_load_streams_per_cycle, unit, weightBuffer);
}
