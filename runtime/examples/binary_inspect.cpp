...
try {
    if (argc < 2)
        throw std::runtime_error(
            ...
            "[--all-queues] [--skip-logical-issues] [--trace schedule.csv] "
            "[--compare reference.ftlpu]");
    bool reportAllQueues = false;
    bool skipLogicalIssues = false;
    ...
    for (int index = 2; index < argc; ++index) {
        ...
        if (argument == "--skip-logical-issues") {
            skipLogicalIssues = true;
            continue;
        }
        ...
        throw std::runtime_error("unknown binary inspector option: " +
                                 std::string(argument));
    }
    ...
    std::cout << "binary target=" << program.target_name
              ...
              << " mxms_per_hemisphere=" << program.hardware.mxms_per_hemisphere
    ...
    if (!skipLogicalIssues) {
        ...
        std::cout << "binary logical_issue_summary queues="
                  ...
                  << " logical_nop_cycles=" << logicalIssues.logical_nop_cycles
                  << '\n';
    }
    ...
    for (const auto& queue : program.queues) {
        const auto macroCount =
            std::ranges::count_if(queue.commands, [](const auto& command) {
        ...
        const bool memPackedQueue =
            binaryVersion >= 28 && macroQueue &&
            queue.kind == ftlpu::software::runtime::QueueKind::Mem;
        const bool mxmPackedQueue =
            binaryVersion >= 33 && macroQueue &&
            (queue.kind == ftlpu::software::runtime::QueueKind::MxmLoad ||
             queue.kind == ftlpu::software::runtime::QueueKind::MxmCompute ||
             queue.kind == ftlpu::software::runtime::QueueKind::MxmDequant);
        const bool packedMacroQueue = memPackedQueue || mxmPackedQueue;
        const bool hasExtendedDescriptor =
            std::ranges::any_of(queue.commands, [](const auto& command) {
                return ftlpu::isa::decode_icu_command_opcode(command.command) ==
                       ftlpu::isa::IcuCommandOpcode::Extended;
            });
        const bool nativeQueue =
            binaryVersion >= 26 && macroCount == 0 && !hasExtendedDescriptor;
        if (memPackedQueue) {
            const auto image =
                ftlpu::software::runtime::encode_mem_macro_bitstream(queue);
            serializedQueueBytes +=
                10 + image.delta_count * 8 + image.bytes.size();
        } else if (mxmPackedQueue) {
            const auto image =
                ftlpu::software::runtime::encode_mxm_macro_bitstream(queue);
            serializedQueueBytes +=
                10 + image.delta_count * 8 + image.bytes.size();
        } else if (macroQueue) {
        ...
        for (const auto& command : queue.commands) {
            ...
            if (packedMacroQueue) {
                ...
                serializedQueueBytes +=
                    1 + command.word_count * sizeof(std::uint32_t) +
            ...
            if (ftlpu::software::runtime::is_vxm_stream_nd_command(command)) {
                ++totalVxmStreamNd;
                const auto stream =
                    ftlpu::software::runtime::decode_vxm_stream_nd_command(
                        command);
                std::size_t points = 1;
                ...
                tracePatternRows +=
                    stream.schedule.rank > 2 ? stream.schedule.counts[2] : 1;
                continue;
            }
            if (ftlpu::software::runtime::is_sxm_tile_program_command(
                ...
                const auto tile =
                    ftlpu::software::runtime::decode_sxm_tile_program_command(
                        command);
                std::size_t points = 1;
                for (std::size_t dimension = 0; dimension < tile.schedule.rank;
                     ++dimension)
                    points *= tile.schedule.counts[dimension];
                sxmTileProgramExpanded += points;
                expandedInstructions += points;
                tracePatternRows +=
                    tile.schedule.rank > 2 ? tile.schedule.counts[2] : 1;
                continue;
            }
            if (ftlpu::software::runtime::is_mem_slice_program_command(
                ...
                const auto sliceProgram =
                    ftlpu::software::runtime::decode_mem_slice_program_command(
                        command);
                totalMemSliceBodyEntries += sliceProgram.body.size();
                maxMemSliceBodyEntries =
                    std::max(maxMemSliceBodyEntries, sliceProgram.body.size());
                memSlicePayloadWords += command.extension_words.size();
                for (const auto& body : sliceProgram.body) {
                    const auto encoded =
                        ftlpu::isa::encode_mem_instruction(body.instruction);
                ...
                tracePatternRows += sliceProgram.body.size() *
                                    (sliceProgram.schedule.rank > 2
                                         ? sliceProgram.schedule.counts[2]
                                         : 1);
                continue;
            }
            if (ftlpu::software::runtime::is_mem_stream_nd_command(command)) {
                ...
                const auto stream =
                    ftlpu::software::runtime::decode_mem_stream_nd_command(
                        command);
                std::size_t points = 1;
                for (std::size_t dimension = 0; dimension < stream.rank;
                     ++dimension)
                    points *= stream.counts[dimension];
                memStreamNdExpanded += points;
                expandedInstructions += points;
                tracePatternRows += stream.rank > 2 ? stream.counts[2] : 1;
                continue;
            }
            ...
            if (ftlpu::software::runtime::is_repeat_2d_command(command)) {
                ++totalRepeat2D;
                const auto repeat =
                    ftlpu::software::runtime::decode_repeat_2d_command(command);
                repeat2DReplayed += repeat.inner_count * repeat.outer_count - 1;
    ...
    const std::size_t encodedWorkEntries =
        totalInstructions + totalRepeats + totalRepeat2D + totalMacros +
        totalMemStreamNd + totalMemSlicePrograms + totalMxmStreamNd +
        totalVxmStreamNd + totalSxmTilePrograms;
    const std::size_t savedWorkEntries =
        expandedInstructions > encodedWorkEntries
            ? expandedInstructions - encodedWorkEntries
            : 0;
    std::cout << "binary aggregate instruction=" << totalInstructions
              << " nop=" << totalNops << " repeat=" << totalRepeats
              << " repeat2d=" << totalRepeat2D << " macro=" << totalMacros
              ...
              << " mem_slice_program_expanded=" << memSliceProgramExpanded
    ...
    if (totalMemSlicePrograms != 0) {
        ...
        std::cout << "binary mem_slice_cost programs=" << totalMemSlicePrograms
    ...
    const auto savedImemWork =
        imem.expanded_work > imem.encoded_work_entries
            ? imem.expanded_work - imem.encoded_work_entries
            : 0;
    const auto workCompression =
        imem.expanded_work == 0 ? 0.0
    ...
    for (const auto kind : {ftlpu::software::runtime::QueueKind::Mem,
        ...
        for (const auto& queue : imem.queues) {
            ...
            encodedWork += queue.instruction_entries + queue.repeat_entries +
                           queue.repeat_2d_entries + queue.macro_entries +
        ...
        if (queueCount == 0)
            continue;
        const auto saved =
            expandedWork > encodedWork ? expandedWork - encodedWork : 0;
        std::cout << "imem resource="
                  << ftlpu::software::runtime::queue_kind_name(kind)
                  << " slot_bits=" << slotBits << " depth_per_queue=" << depth
                  << " queues=" << queueCount << " used_slots=" << usedSlots
    ...
    for (const auto& queue : imem.queues) {
        if (!reportAllQueues && !queue.overflow())
            continue;
        std::cout << "imem queue resource="
                  ...
                  << " utilization_percent="
                  << 100.0 * static_cast<double>(queue.used_slots) / queue.depth
    ...
    std::cout << "imem model=target-physical-v1"
              ...
              << " mxm_delta_rle_queues=" << physical.mxm_delta_rle_queues
              ...
              << " peak_macro_context_bits=" << physical.peak_macro_context_bits
    ...
    for (const auto kind : {ftlpu::software::runtime::QueueKind::Mem,
        ...
        for (const auto& queue : physical.queues) {
            ...
            contextCapacity =
                std::max(contextCapacity, queue.macro_context_capacity);
            peakContextBits +=
                static_cast<std::uint64_t>(queue.peak_macro_contexts) *
                queue.macro_context_bits;
            provisionedContextBits +=
                static_cast<std::uint64_t>(queue.macro_context_capacity) *
                queue.macro_context_bits;
        ...
        std::cout << "imem physical_resource="
                  << ftlpu::software::runtime::queue_kind_name(kind)
                  << " queues=" << queues << " used_slots=" << slots
                  << " max_used_slots=" << maxSlots << " used_bits=" << bits
                  << " peak_macro_contexts=" << peakContexts
                  << " macro_context_capacity=" << contextCapacity
                  << " peak_context_bits=" << peakContextBits
                  << " provisioned_context_bits=" << provisionedContextBits
                  << " template_runs=" << runs
                  << " compact_template_runs=" << compactTemplateRuns
                  << " extended_template_runs=" << extendedTemplateRuns
                  << " escaped_deltas=" << escapes << '\n';
    }
    std::map<std::pair<std::uint32_t, ftlpu::software::runtime::QueueKind>,
             std::size_t>
        scaleRelocations;
    for (const auto& relocation : program.scale_relocations)
        ++scaleRelocations[{relocation.binding_index, relocation.queue_kind}];
    std::cout << "binary scale_relocations=" << program.scale_relocations.size()
              << '\n';
    ...
    const auto reportStreamReleases =
        [&](std::string_view direction, std::size_t offset, std::size_t count) {
    ...
    if (program.stream_release_cycles.size() ==
        ...
        reportStreamReleases("west", directionalStreams, directionalStreams);
    } else {
        reportStreamReleases("merged", 0, program.stream_release_cycles.size());
    }
    const std::size_t reported = reportAllQueues
                                     ? queues.size()
                                     : std::min<std::size_t>(queues.size(), 20);
    for (std::size_t i = 0; i < reported; ++i) {
        ...
        std::cout << "binary queue_occupancy rank=" << i << " resource="
                  << ftlpu::software::runtime::queue_kind_name(queue.kind)
                  << " queue=" << queue.index
                  << " commands=" << queue.commands.size()
                  << " instruction=" << instructions << " nop=" << nops
                  << " repeat=" << repeats << " repeat2d=" << repeats2d
                  << " macro=" << macros << " mem_stream_nd=" << memStreamNd
                  << " mem_slice_program=" << memSlicePrograms
                  << " mxm_stream_nd=" << mxmStreamNd
                  << " vxm_stream_nd=" << vxmStreamNd
                  << " sxm_tile_program=" << sxmTilePrograms;
        const auto physicalQueue =
            std::ranges::find_if(physical.queues, [&](const auto& candidate) {
        ...
        if (physicalQueue != physical.queues.end())
            std::cout << " physical_slots=" << physicalQueue->physical_slots
                      << " payload_bits=" << physicalQueue->payload_bits
                      ...
                      << " context_overflow="
                      << (physicalQueue->macro_context_overflow() ? "yes"
                                                                  : "no");
        std::cout << '\n';
    }
    for (const auto kind : {ftlpu::software::runtime::QueueKind::Mem,
        ...
        if (kindQueues != 0)
            std::cout << "binary resource_summary resource="
                      << ftlpu::software::runtime::queue_kind_name(kind)
                      << " queues=" << kindQueues
                      << " commands=" << kindCommands << " min=" << kindMin
                      << " max=" << kindMax << " average="
                      << static_cast<double>(kindCommands) / kindQueues << '\n';
        const auto found = std::ranges::find_if(
            queues, [kind](const auto* queue) { return queue->kind == kind; });
        if (found == queues.end())
            continue;
        ...
        std::cout << "binary resource_max resource="
                  ...
                  << " instruction=" << instructionCount << " nop=" << nopCount
                  << " repeat=" << repeatCount << " repeat2d=" << repeat2DCount
                  << " macro=" << macroCount
    ...
    for (const auto& binding : program.bindings) {
        std::cout << "binary binding index=" << binding.index
                  ...
                  << " role=" << binding.role << " name=" << binding.name
    ...
    ftlpu::software::runtime::print_runtime_performance(program, cycles,
                                                        std::cout);
    if (tracePath)
        ftlpu::software::runtime::write_schedule_trace_csv(program, *tracePath);
    if (comparePath) {
        ...
        const auto comparison =
            ftlpu::software::runtime::compare_logical_issues(program,
                                                             reference);
            ...
            std::cout << " left="
                      << (mismatch.left
                              ? ftlpu::software::runtime::
                                    describe_logical_instruction(*mismatch.left)
                              : "NOP")
                      << " right="
                      << (mismatch.right ? ftlpu::software::runtime::
